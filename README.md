...
Raw-dependent workflows should rely on that structure.
...
The tracked base wiki contains reusable Markdown content, templates, empty
placeholder folders, and minimal ignore rules only. OMW runtime files such as
`.omw/contract.json`, search indexes, ingest drafts, editor-local settings, and
report/validation logic are not tracked or packed with the repository base wiki.
If you use the repository `.wiki` directly as the active wiki, those runtime
files may be generated locally under `.wiki/.omw/`; `.wiki/.gitignore` keeps
them out of Git and the npm package.
...
| `omw wiki capture --title "<title>" [--type agent_session|discussion] [--stdin|--body <text>] [--dry-run] [--json]` | Wiki-scoped form of Raw note capture. |
...
| `omw wiki daily --author <name> --team <team> --date YYYY-MM-DD [--stdin|--body <text>] [--dry-run] [--json]` | Wiki-scoped daily report Raw note creation. |
...
| `omw capture --title "<title>" [--type agent_session|discussion] [--stdin|--body <text>] [--dry-run] [--json]` | Capture an agent-session or discussion Raw note. |
...
| `omw daily --author <name> --team <team> --date YYYY-MM-DD [--stdin|--body <text>] [--dry-run] [--json]` | Create or update a daily report Raw note. |
